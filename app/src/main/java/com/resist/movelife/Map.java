...
public class Map extends Activity implements LocationListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		ArrayAdapter<String> adapter = new ArrayAdapter<String>(getBaseContext(), R.layout.drawer_list_item, getResources().getStringArray(R.array.categorieën));
	...
	public void getMarkers() {
		...
		for(Company store : array) {
			...
			MarkerOptions marker = new MarkerOptions().position(l);
			if(store.hasEvent()) {
		...
		for(User friend : friends) {
			...
			MarkerOptions marker = new MarkerOptions().position(l).icon(BitmapDescriptorFactory.fromResource(R.drawable.ic_map_friend)).title((friend.getName() != null ? friend.getName() : friend.getEmail())).snippet((friend.getLastSeen() != null ? friend.getLastSeen().toString() : ""));
	...
	private void getTypeMarker(final List<Company> array, int resource, int resourceEvent) {
		...
		for(Company store : array) {
			LatLng l = new LatLng(store.getLatitude(), store.getLongitude());
			MarkerOptions marker = new MarkerOptions().position(l).title(store.getName()).snippet("" + store.getRating());
			if(store.hasEvent()) {
